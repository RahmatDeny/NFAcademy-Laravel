...
@section('content')
    <div class="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow">
        ...
        <div class="bg-gray-50 px-6 py-3 border-b border-gray-200 flex flex-wrap gap-4 items-center">
            <span class="text-sm text-gray-600" id="genreInfo">Showing 1 to 10 of 0 results</span>
            <div class="flex items-center gap-2 ml-auto">
                ...
                <button id="toggleGenreCreate" class="ml-4 px-4 py-2 rounded-md bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700">Tambah</button>
            </div>
        </div>

        <!-- Create Modal -->
        <div id="genreModal" class="fixed inset-0 hidden bg-black/40 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg shadow-xl w-full max-w-lg">
                <div class="flex items-center justify-between px-6 py-3 border-b">
                    <h3 class="text-lg font-semibold">Tambah Genre</h3>
                    <button id="closeGenreModal" class="text-gray-500 hover:text-gray-700">✕</button>
                </div>
                <div class="px-6 py-4">
                    <form id="genreCreateForm" class="grid gap-4 md:grid-cols-2">
                        <div>
                            <label class="block text-sm text-gray-700 mb-1">Nama Genre</label>
                            <input id="genreName" type="text" required class="w-full border rounded px-3 py-2" placeholder="Nama genre">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm text-gray-700 mb-1">Deskripsi</label>
                            <textarea id="genreDescription" required class="w-full border rounded px-3 py-2" rows="3" placeholder="Deskripsi"></textarea>
                        </div>
                        <div class="md:col-span-2 flex items-center gap-2">
                            <button type="submit" class="px-4 py-2 rounded bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700">Simpan</button>
                            <button type="button" id="cancelGenreCreate" class="px-4 py-2 rounded border text-sm font-semibold hover:bg-gray-50">Batal</button>
                            <span id="genreCreateMsg" class="text-sm ml-2"></span>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody id="genreTable" class="divide-y divide-gray-200"></tbody>
            </table>
        </div>
        ...
        <div class="bg-gray-50 px-6 py-4 border-t border-gray-200 flex justify-end gap-2">
            ...
            <div id="genrePageBtns" class="flex gap-1"></div>
...
<script>
    ...
    class Paginator {
        ...
        setupListeners() {
            ...
            document.getElementById(this.nextBtnId).addEventListener('click', () => {
                if (this.currentPage < this.getTotalPages()) { this.currentPage++; this.render(); }
            });
        }
        ...
        renderPagination() {
            ...
            pageBtnsContainer.querySelectorAll('button').forEach((btn, idx) => {
                btn.addEventListener('click', () => { this.currentPage = idx + 1; this.render(); });
            });
    ...
    const genrePaginator = new Paginator(dataGenres, 'genreTable', 'genrePerPage', 'genreInfo', 'genrePrevBtn', 'genreNextBtn', 'genrePageBtns');

    // Create handlers (Modal)
    const genreModal = document.getElementById('genreModal');
    const openGenre = () => { genreModal.classList.remove('hidden'); };
    const closeGenre = () => {
        genreModal.classList.add('hidden');
        document.getElementById('genreCreateForm').reset();
        document.getElementById('genreCreateMsg').textContent = '';
    };
    document.getElementById('toggleGenreCreate').addEventListener('click', openGenre);
    document.getElementById('closeGenreModal').addEventListener('click', closeGenre);
    document.getElementById('cancelGenreCreate').addEventListener('click', closeGenre);
    genreModal.addEventListener('click', (e) => { if (e.target === genreModal) closeGenre(); });
    document.getElementById('genreCreateForm').addEventListener('submit', async (e) => {
        e.preventDefault();
        const payload = {
            name: document.getElementById('genreName').value.trim(),
            description: document.getElementById('genreDescription').value.trim()
        };
        const msg = document.getElementById('genreCreateMsg');
        msg.textContent = 'Menyimpan...'; msg.className = 'text-sm text-gray-500';
        try {
            const res = await fetch('/api/genres', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify(payload)});
            const data = await res.json();
            if (!res.ok) { throw data; }
            // Sukses: tambahkan ke data & render ulang
            dataGenres.push(data.data);
            genrePaginator.render();
            document.getElementById('genreCreateForm').reset();
            msg.textContent = 'Tersimpan'; msg.className = 'text-sm text-emerald-600';
            setTimeout(() => { closeGenre(); }, 700);
        } catch (err) {
